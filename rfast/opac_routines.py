...
def cia_read(species, lam, opdir):
    ...
    ciaid0 = np.transpose(
        np.array([['     '] * (max(ncia0) + 1) for i in range(ngas)]))
    ciaid0[0:2, 0] = ['co2', 'co2']
    ciaid0[0:3, 1] = ['h2', 'h2', 'he']
    ciaid0[0:2, 2] = ['n2', 'n2']
    ciaid0[0:3, 3] = ['o2', 'o2', 'n2']

    # determine number of cia cases
    icia = 0
    ncia = [0] * len(species)
    ciaid = np.transpose(
        np.array([['     '] * (max(ncia0) + 1) for i in range(len(species))]))
    ...
    for isp in range(len(species)):
        ...
        if (species[isp].lower() == 'co2'):
            ...
            cia_interp = interpolate.interp1d(
                temp0, cia0, axis=0, assume_sorted=True, fill_value="extrapolate")
            cia0 = cia_interp(temp)

            # interpolate onto lam
            cia_interp = interpolate.interp1d(
                lam0, cia0, axis=1, fill_value="extrapolate")
...
def opacities_read(species, lam, opdir):
    ...
    if (Nspec > 0):
        ...
        sig_interp = interpolate.interp1d(
            wn0[:-2], np.log10(sig[:, :, :-2]), axis=2, assume_sorted=True, fill_value="extrapolate")
        ...
        if (xsc[isp] == 'y'):
            ...
            xsec_interp = interpolate.interp1d(lamx, np.log10(
                xsecx), assume_sorted=True, fill_value="extrapolate")
        ...
        for j in range(1, Nspec):
            ...
            if (xsc[isp] == 'y'):
                ...
                xsec[np.where(xsec < smallsig)] = smallsig
                xsec = np.repeat(xsec[np.newaxis, :],   sigma.shape[1], axis=0)
                xsec = np.repeat(xsec[:, np.newaxis, :],
                                 sigma.shape[2], axis=1)
...
def rayleigh(lam, ray0, f, fb, rayb):
    ...
    sigma = np.multiply.outer(
        np.sum(f * ray0[:, np.newaxis], axis=0), (0.4579 / lam)**4.)
    sigma = sigma + \
        np.multiply.outer(fb * rayb * ray0[0, np.newaxis], (0.4579 / lam)**4.)
...
def init_cloud_optics(lam, g1, g2, g3, w, lamc0, grey, cld, opdir):
    ...
    if cld:
        ...
        if grey:
            ...
            # liquid
            data = ascii.read(opdir + 'strato_cum.mie',
                              data_start=20, delimiter=' ')
            lam_in = data['col1']
            w_in = data['col10']
            g_in = data['col11']
            q_in = data['col7']
            w_interp = interpolate.interp1d(
                lam_in, w_in, assume_sorted=True, fill_value="extrapolate")
            g_interp = interpolate.interp1d(
                lam_in, g_in, assume_sorted=True, fill_value="extrapolate")
            q_interp = interpolate.interp1d(
                lam_in, q_in, assume_sorted=True, fill_value="extrapolate")
            wcl = w_interp(lam)
            gcl = g_interp(lam)
            qcl = q_interp(lam) / q_interp(lamc0)
            # ice
            data = ascii.read(opdir + 'baum_cirrus_de100.mie',
