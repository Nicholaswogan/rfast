...
def set_gas_info(bg):
    ...
    mmw0 = np.float32([39.948,    16.04,    44.01, 2.0159,   18.015,
                      4.0026,   28.013,  31.999, 48.000, 44.013,  28.01])
    ray0 = np.float32([8.29e-31, 2.035e-30, 2.454e-30, 2.16e-31,
                      8.93e-31, 1.28e-32, 10.38e-31, 8.39e-31, 0.0000, 0.0000, 0.0000])
    nu0 = np.float32([0.000281, 0.000444, 0.000449, 0.000132, 0.000261,
                     0.000035, 0.000298, 0.000271, 0.0000, 0.0000, 0.0000])
...
def hydrostat(Nlev, p, t, m, Mp, Rp, gp, ip=-1):
    ...
    if (ip == -1):
        ...
        for i in range(1, Nlev):
            k = Nlev - i - 1
            a = kB / grav0 / mm[k]
            fac = a * ((t[k + 1] - (t[k] - t[k + 1]) / (p[k] - p[k + 1])
                       * p[k + 1]) * np.log(p[k + 1] / p[k]) - (t[k] - t[k + 1]))
    ...
    # case where Rp, gp apply somewhere else in profile
        ...
        for i in range(1, ip + 1):
            k = ip - i
            a = kB / grav0 / mm[k]
            fac = a * (t[k + 1] - (t[k] - t[k + 1]) /
                       (p[k] - p[k + 1])) * np.log(p[k + 1] / p[k])
            fac = fac - a * ((t[k] - t[k + 1]) /
                             (p[k] - p[k + 1]) * (p[k] - p[k + 1]))
            fac = (Rp * Re) / (1 + (z[k + 1] / Rp / Re)) - fac
            z[k] = (Rp * Re / fac - 1) * Rp * Re
    ...
    if cld:
        ...
        if (src != 'phas'):
            ...
            if (tauc0 <= taus):
                ...
                p0 = np.logspace(
                    np.log10(pmin), np.log10(pmax), Nlev - Npc - 1)
                if (pmin == pt):
                    p0[0] = 0.5 * (p0[0] + p0[1])
                if (pmax == pt + dpc):
                    p0[-1] = 0.5 * (p0[-1] + p0[-2])
