...
# log-prior function
def lnprior(x):
  xiN2,xiO2,xiH2O,xiCO2,xiO3,xiCH4,lpmax,lAs,lRp,lgp,ldpc,lpt,ltauc0,lfc = x
  pmax,As,Rp,gp,dpc,pt,tauc0,fc = 10**(lpmax),10**(lAs),10**(lRp),10**(lgp),10**(ldpc),10**(lpt),10**(ltauc0),10**(lfc)
  # no retrieved gases or performing clr retrieval
  ...
  # prior limits
  if ximin <= xiN2 <= ximax and ximin <= xiO2 <= ximax and ximin <= xiH2O <= ximax and ximin <= xiCO2 <= ximax and ximin <= xiO3 <= ximax and ximin <= xiCH4 <= ximax and 1.0 <= pmax <= 100000000.0 and 0.01 <= As <= 1.0 and 0.1 <= Rp <= 10.0 and 1.0 <= gp <= 25.0 and 1.0 <= dpc <= 100000000.0 and 1.0 <= pt <= 100000000.0 and 0.001 <= tauc0 <= 1000.0 and 0.001 <= fc <= 1.0 and  pb <= pmax:
    return 0.0 + lng
  return -np.inf
...
def lnlike(x):
  ...
  xiN2,xiO2,xiH2O,xiCO2,xiO3,xiCH4,lpmax,lAs,lRp,lgp,ldpc,lpt,ltauc0,lfc = x
  xi   = np.float32([xiN2,xiO2,xiH2O,xiCO2,xiO3,xiCH4,])
  clrs = np.sum(np.exp(xi)) + np.exp(-np.sum(xi))
  fN2,fO2,fH2O,fCO2,fO3,fCH4,pmax,As,Rp,gp,dpc,pt,tauc0,fc = np.exp(xiN2)/clrs,np.exp(xiO2)/clrs,np.exp(xiH2O)/clrs,np.exp(xiCO2)/clrs,np.exp(xiO3)/clrs,np.exp(xiCH4)/clrs,10**(lpmax),10**(lAs),10**(lRp),10**(lgp),10**(ldpc),10**(lpt),10**(ltauc0),10**(lfc)
...
if __name__ == '__main__':
  ...
  xiN2,xiO2,xiH2O,xiCO2,xiO3,xiCH4 = np.log(f0[species_r=='n2'][0]/gx),np.log(f0[species_r=='o2'][0]/gx),np.log(f0[species_r=='h2o'][0]/gx),np.log(f0[species_r=='co2'][0]/gx),np.log(f0[species_r=='o3'][0]/gx),np.log(f0[species_r=='ch4'][0]/gx)
  ...
  guess = [xiN2,xiO2,xiH2O,xiCO2,xiO3,xiCH4,lpmax,lAs,lRp,lgp,ldpc,lpt,ltauc0,lfc]
  ndim  = len(guess)
