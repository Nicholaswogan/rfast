...
def read_yaml_input(scr_file):
  ...
  for key in REQUIRED_KEYS:
    if key not in keys:
      raise Exception('Missing key "'+key+'" in input file "'+scr_file+'"')
    else:
  ...
  if data['src'] not in src_options:
    raise Exception('"src" must be one of the following: '+str(src_options))
  ...
  if data['bg'] not in bg_options:
    raise Exception('"bg" must be one of the following: '+str(bg_options))
    
  species_r_options = ['ar', 'n2', 'o2', 'h2o', 'o3', 'co2', 'ch4', 'h2', 'he', 'n2o', 'co']
  ...
  if (data['lams'].shape[0] > 1 and data['lams'].shape[0] != data['res'].shape[0]):
    raise Exception("rfast warning | major | smpl length inconsistent with wavelength grid")
  ...
  if (data['smpl'].shape[0] > 1 and data['smpl'].shape[0] != data['res'].shape[0]):
    raise Exception("rfast warning | major | smpl length inconsistent with resolution grid")
  ...
  if (data['snr0'].shape[0] > 1 and data['snr0'].shape[0] != data['res'].shape[0]):
    raise Exception("rfast warning | major | snr0 length inconsistent with wavelength grid")
  ...
  if (data['lam0'].shape[0] > 1 and data['lam0'].shape[0] != data['res'].shape[0]):
    raise Exception("rfast warning | major | lam0 length inconsistent with wavelength grid")
  ...
  if (data['lam0'].shape[0] > 1):
    for i in range(data['lam0'].shape[0]):
      if (data['lam0'][i] < min(data['lams']) or data['lam0'][i] > max(data['laml'])):
        raise Exception("rfast warning | major | lam0 outside wavelength grid")
  else:
  ...
  if (not os.path.isdir(data['opdir'])):
    raise Exception("rfast warning | major | opacities directory does not exist")
  ...
  if (np.sum(data['f0']) - 1 > 1.e-6 and not data['rdgas']):
    if (np.sum(data['f0']) - 1 < 1.e-3):
      warning.warn("rfast warning | minor | input gas mixing ratios sum to slightly above unity")
    else:
      raise Exception("rfast warning | major | input gas mixing ratios sum to much above unity")
  
  if 'gp' not in data and 'Mp' not in data:
    raise Exception("Planet mass ('Mp') or gravity ('gp') must be specified.")
  elif 'gp' in data and 'Mp' in data:
    raise Exception("rfast warning | major | cannot independently set planet mass and gravity in inputs")
  ...
  if (data['pt']+data['dpc'] > data['pmax']):
    raise Exception("rfast warning | major | cloud base below bottom of atmosphere")

  # transit radius pressure cannot be larger than max pressure
  if (data['p10'] > data['pmax']):
    raise Exception("rfast warning | major | transit radius pressure below bottom of atmosphere")
