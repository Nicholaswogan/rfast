...
if len(sys.argv) >= 2:
    ...
    filename_scr = input("rfast inputs script filename: ")
...
fnr, fnn, fns, dirout, Nlev, pmin, pmax, bg,\
    ...
    Ts, Rs,\
    ntype, snr0, lam0, rnd,\
    clr, fmin, mmr, nwalkers, nstep, nburn, thin, restart, progress = inputs(
        filename_scr)
...
# no. of res elements to extend beyond grid edges to avoid edge sensitivity (2--4)
Nres = 3
lam, dlam = gen_spec_grid(lams, laml, np.float_(res), Nres=0)
lam_hr, dlam_hr = gen_spec_grid(lams, laml, np.float_(
    res) * smpl, Nres=np.rint(Nres * smpl))

# initialize opacities and convolution kernels
sigma_interp, cia_interp, ncia, ciaid, kern = init(
    lam, dlam, lam_hr, species_l, species_c, opdir, pf, tf)
...
# timing
tend = time.time()
print('Total setup and forward model timing (s), spectral points: ',
      tend - tstart, lam_hr.shape[0])
...
ascii.write(data_out, dirout + fns + '.raw',
